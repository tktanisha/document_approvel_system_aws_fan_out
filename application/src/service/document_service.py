...
class DocumentService:
    def __init__(self,document_repo:DocumentRepo,audit_repo:AuditRepo,user_repo : UserRepo, event_publisher_service:EventPublisher):
    ...
    async def get_all_document(self, user_ctx, status: Optional[str] = None)-> List[Document]:
        ...
        try:
            docs:List[Document] = await self.document_repo.get_documents(user_ctx, status)
            return docs
    ...
    async def update_status(self, user_ctx: dict, document_id: str,new_status: DocumentStatus,comment: Optional[str]) -> Document:
